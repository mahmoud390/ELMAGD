...
namespace Elmagd
{
    public partial class Full_Production : Form
    {
        ...
        {
            conn.Open();
            cmd.CommandText = @"select TEMP_FULLPRODUCTION.id,STORE.name as المخزن,CATEGORY.name as الصنف,TEMP_FULLPRODUCTION.quantity as الكمية,QUANTITY_TYPE.name as نوع_الكمية,TEMP_FULLPRODUCTION.notes from TEMP_FULLPRODUCTION inner join STORE on STORE.id =TEMP_FULLPRODUCTION.store_id inner join CATEGORY on CATEGORY.id = TEMP_FULLPRODUCTION.cat_id inner join QUANTITY_TYPE on QUANTITY_TYPE.id = TEMP_FULLPRODUCTION.quantitytype_id ";
            cmd.Connection = conn;
        ...
        private void FullProductionBindGrid()
        {
            conn.Close();
            conn.Open();
            cmd.CommandText = @"select FULLPRODUCTION.id,STORE.name as المخزن,CATEGORY.name as الصنف,FULLPRODUCTION.quantity as الكمية,QUANTITY_TYPE.name as نوع_الكمية ,FULLPRODUCTION.notes from FULLPRODUCTION inner join STORE on STORE.id =FULLPRODUCTION.store_id inner join CATEGORY on CATEGORY.id = FULLPRODUCTION.cat_id inner join QUANTITY_TYPE on QUANTITY_TYPE.id = FULLPRODUCTION.quantitytype_id ";
            cmd.Connection = conn;
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
