...
namespace Elmagd
{
    public partial class Suppliers_Detail : Form
    {
        ...
        int firstLoad = 0;
        double supplier_invoice_paid,payment_suppliers_value;
        ...
        private void btnCalc_Click(object sender, EventArgs e)
        {
            ...
            if (dateFromDummy == dateToDummy)
                ...
                cmd.CommandText = @"select sum(paid) from SUPPLIERS_INVOICE where SUPPLIERS_INVOICE.date between '" + dateFrom.Value.Date.ToShortDateString() + "'" + " and '" + dateTo.Value.Date.ToShortDateString() + "' and ";
            cmd.Connection = conn;
            ...
            if (dateFromDummy == dateToDummy)
                ...
                cmd.CommandText = @"select sum(value) from PAYMENTS_SUPPLIERS where PAYMENTS_SUPPLIERS.date between '" + dateFrom.Value.Date.ToShortDateString() + "'" + " and '" + dateTo.Value.Date.ToShortDateString() + "' and ";
            cmd.Connection = conn;
        ...
        private void btnShowDuringPeriod_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                conn.Open();
                if (dateFromDummy == dateToDummy)
                    cmd.CommandText = @"select PAYMENTS_SUPPLIERS.value المدفوع_للمورد,PAYMENTS_SUPPLIERS.date التاريخ,SUPPLIERS.name إسم_المورد  from PAYMENTS_SUPPLIERS inner join SUPPLIERS on PAYMENTS_SUPPLIERS.suppliers_id=SUPPLIERS.id ";
                else
                    cmd.CommandText = @"select PAYMENTS_SUPPLIERS.value المدفوع_للمورد,PAYMENTS_SUPPLIERS.date التاريخ,SUPPLIERS.name إسم_المورد  from PAYMENTS_SUPPLIERS inner join SUPPLIERS on PAYMENTS_SUPPLIERS.suppliers_id=SUPPLIERS.id  where PAYMENTS_SUPPLIERS.date between '" + dateFrom.Value.Date.ToShortDateString() + "'" + " and '" + dateTo.Value.Date.ToShortDateString() + "'";
                cmd.Connection = conn;
